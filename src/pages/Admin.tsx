...
import { 
  ...
  Video, Image as ImageIcon, Loader2, Upload
} from "lucide-react";
...
export default function Admin() {
  const [config, setConfig] = useState({
    instagram: "https://www.instagram.com/madad_ai_tj?igsh=MW82aG42bDk0bjc0bA%3D%3D&utm_source=qr",
  ...
  const handleSaveAd = () => {
    ...
    localStorage.setItem("madad_promo_ad", JSON.stringify({
      ...adForm,
      updatedAt: Date.now()
    }));
    toast.success("Реклама бомуваффақият захира шуд!");
  };
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hidden">
        {[
          { id: "overview", icon: <BarChart3 />, label: "Омор" },
          { id: "settings", icon: <Settings />, label: "Танзимоти платформа" },
          { id: "users", icon: <Users />, label: "Идоракунии корбарон" },
          { id: "ads", icon: <Activity />, label: "Реклама" }
        ].map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap ${
              activeTab === tab.id ? "glass admin-gradient text-medical-green border-medical-green/50" : "text-gray-500 hover:text-gray-300"
            }`}
          >
            {tab.icon}
            {tab.label}
        ...
        {activeTab === "overview" && (
          <>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 glass rounded-[32px] p-8 space-y-6">
              ...
              <div className="flex justify-between text-[10px] uppercase font-bold text-gray-600 px-2">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-3 glass rounded-[32px] p-10 space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase ml-1">Истиноди Инстаграм</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase ml-1">Истиноди Телеграм Бот</label>
                    <div className="relative">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase ml-1">Модели СУ</label>
                    <select 
                    ...
                    >
                      <option value="gemini-3-flash-preview">Gemini 3 Flash (Зуд)</option>
                      <option value="gemini-3.1-pro-preview">Gemini 3.1 Pro (Коршинос)</option>
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase ml-1">Хатти доимии дастгирӣ</label>
                    <input 
        ...
        {activeTab === "ads" && (
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-3 glass rounded-[32px] p-10 space-y-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-6">
                  <div className="space-y-4">
                    <label className="text-xs font-bold uppercase tracking-widest text-gray-500">Матни реклама</label>
                    <textarea 
                  ...
                  <div className="space-y-4">
                    <label className="text-xs font-bold uppercase tracking-widest text-gray-500">Боргузории медиа (Акс ё Видео)</label>
                    <div className="relative group cursor-pointer">
                      ...
                      <div className="w-full bg-white/5 border-2 border-dashed border-white/10 rounded-2xl p-10 flex flex-col items-center justify-center gap-4 group-hover:border-medical-green/50 transition-all">
                        ...
                        <span className="text-[10px] text-gray-600">Максимум: 2MB</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <label className="text-xs font-bold uppercase tracking-widest text-gray-500">Пешнамоиш</label>
                  <div className="glass rounded-[2rem] aspect-video w-full overflow-hidden relative flex items-center justify-center bg-black/40">
                    {adForm.mediaUrl ? (
                      ...
                      <div className="text-gray-600 text-sm font-bold uppercase tracking-widest">Медиа вуҷуд надорад</div>
                    )}
                    {adForm.text && (
